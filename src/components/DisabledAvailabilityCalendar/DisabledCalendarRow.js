...
export default function DisabledCalendarRow({ days, isCurrentMonth }) {
  ...
  const handleMorningClick = () => {
    ...
    if (confirmedStart && compareDates(selected, confirmedStart.day)) {
      ...
      if (confirmedStart?.pm && confirmedEnd) {
        dispatch({
          type: 'setConfirmedStart',
          data: { day: selected, am: true },
        })
      }
    ...
    if (
      confirmedStart &&
      selected < confirmedStart.day &&
      confirmedEnd.sameTimeSlot
    ) {
      dispatch({ type: 'setConfirmedStart', data: { day: selected, am: true } })
      dispatch({
        type: 'setConfirmedEnd',
        data: { day: selected, am: true, sameTimeSlot: true },
      })
      return
    }
  ...
  const validateTimeSlots = ({ am }) => {
    ...
    const timeSlot = yearAvailability.slice(
      startIndex * 2 + (confirmedStart?.am ? 1 : 2),
      endIndex * 2 + (am ? 2 : 3)
    )
  ...
  const handleAfternoonClick = () => {
    ...
    if (
      ...
      dispatch({
        type: 'setConfirmedEnd',
        data: { day: selected, pm: true, sameTimeSlot: true },
      })
      return
    }
    if (confirmedStart && selected < confirmedStart.day) {
  ...
  const handleMorningLogic = () => {
    if (
      (confirmedStart &&
        selected &&
        compareDates(selected, confirmedStart.day) &&
        confirmedStart?.am) ||
      (confirmedEnd &&
        selected &&
        compareDates(selected, confirmedEnd.day) &&
        confirmedEnd?.am)
    ) {
