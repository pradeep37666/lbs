...
export const handleEmail = (e, setEmail, setValidation) => {
  ...
  if (emailInput.length === 0) {
    ...
    Instance.get(`/user/checkExist/?email=${emailInput}`)
      .then(response => {
        if (response.status === 204) {
          ...
          setEmail('')
          setValidation(
            'This user already exists, please use another email address'
          )
        }
      })
      .catch(error => {
    ...
    setEmail('')
    setValidation(
      'Incorrect email format, should be in format: example@example.com'
    )
  }
}
...
export const handlePhoneNumber = (e, setPhone, setValidation) => {
  ...
  if (phoneInput.length === 0) {
    ...
    Instance.get(`/user/checkExist/?mobile=${phoneInput}`)
      .then(response => {
        if (response.status === 204) {
          setPhone(phoneInput)
          setValidation('')
...
export const handlePassword = (e, setPassword, setValidation) => {
  ...
  if (
    ...
    setPassword('')
    setValidation(
      'Password must be at least 8 characters, contain 2 numbers and a special character'
    )
  }
}

export const handlePasswordConfirm = (
  e,
  setConfirmPassword,
  setValidation,
  password
...
export const handleCardNumber = (e, setCardNumber, setValidation) => {
  ...
  if (numberInput.length === 0) {
    ...
    setCardNumber('')
    setValidation(
      'Incorrect format for card number, should be in the format: 1234567890123456'
    )
  }
}
...
export const handleExpiry = (e, setExpiry, setValidation) => {
  ...
  if (expiryInput.length === 0) {
    ...
    setExpiry('')
    setValidation(
      'Incorrect format for card number, should be in the format: mm/yy, mmyy or mmyyyy'
    )
  }
}
...
export const handleAddress = (e, setAddress, setValidation) => {
  ...
  if (addressInput.length === 0) {
    ...
    setAddress('')
    setValidation(
      'Incorrect address format, should be in format: 12 Example Street'
    )
  }
}
...
export const handleCountry = (e, setCountry, setValidation) => {
  ...
  if (countryInput.length === 0) {
    ...
    /^([^0-9!@#$%^&*()]*)$/.test(countryInput) &&
    countryInput.length >= 4
  ) {
