...
export default function Register() {
  ...
  const getUserDetails = () => {
    if (isLenderUpgrade) {
      ...
      if (address.country === 'AU' || 'Australia') {
        lenderDetails.stripeDetails.bsb = BSB
      }
  ...
  const registerUser = async () => {
    ...
    try {
      const { data, status } = await Instance.post(
        isLenderUpgrade ? '/auth/lender/signUp' : '/auth/borrower/signUp',
        signupDetails
      )
      if (status === 201) {
      ...
      <div className='RegistrationWrapper'>
        <div className='LoginMain'>
          ...
          <div className='LoginText'>
            You have successfully created your Little Big Shed account and are
            now ready to start borrowing!
          </div>
          <button
    ...
    <FormContext.Provider value={{ state, dispatch }}>
      <PageWrapper>
        <Banner
          ...
          prevPage={() =>
            getPrevRegisterPage(isLenderUpgrade, currentPage, dispatch, history)
          }
        />
        {renderCurrentPage()}
      </PageWrapper>
